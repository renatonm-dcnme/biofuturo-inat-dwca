...
p <- ggplot(lorenz_curves, aes(x = p, y = L, color = period_en)) +
  ...
  labs(
    x = "Cumulative share of citizen scientists (%)",
...
png_path  <- file.path(out_dir, "10_lorenz_gini_by_city_period.png")
tiff_path <- file.path(out_dir, "10_lorenz_gini_by_city_period.tiff")
eps_path  <- file.path(out_dir, "10_lorenz_gini_by_city_period.eps")

ggsave(png_path, p, width = 8, height = 6, units = "in", dpi = 300, bg = "white")
...
ggsave(eps_path, p, device = eps_device, width = 8, height = 6, units = "in")
