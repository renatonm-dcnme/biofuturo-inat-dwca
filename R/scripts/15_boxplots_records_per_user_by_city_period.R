...
user_contrib_plot <- user_contrib %>%
  anti_join(outlier_to_drop,
            by = c("municipio_tag", "period_en", "user.id", "n_records"))

cat("\n### Dimensão da base user_contrib_plot (sem o outlier extremo para o gráfico) ###\n")
print(dim(user_contrib_plot))

user_records_summary <- user_contrib_plot %>%
...
print(user_records_summary)

out_dir <- "outputs"
fs::dir_create(out_dir)

readr::write_csv(
  user_records_summary,
  file.path(out_dir, "15_user_records_summary_by_city_period.csv")
)

cat("\nArquivo de resumo salvo em: outputs/15_user_records_summary_by_city_period.csv\n")
...
y_max_global <- max(user_contrib_plot$n_records, na.rm = TRUE)

n_labels <- user_contrib_plot %>%
  group_by(municipio_tag, period_en) %>%
  summarise(
    n_users = n(),
    .groups = "drop"
...
p_box <- ggplot(
  ...
  geom_boxplot(
    data  = dplyr::filter(user_contrib_plot, period_en != "During"),
  ...
  scale_y_continuous(name = "Records per unique user") +
  expand_limits(y = y_max_global * 1.12) +
  labs(
    title = "Number of iNaturalist records per user by municipality and period"
...
ggsave(
  filename = file.path(out_dir, "15_boxplots_records_per_user_by_city_period.png"),
  plot     = p_box,
  width    = 8,
  height   = 6,
  dpi      = 300
...
cat("\nFigura salva em: outputs/15_boxplots_records_per_user_by_city_period.png\n")
