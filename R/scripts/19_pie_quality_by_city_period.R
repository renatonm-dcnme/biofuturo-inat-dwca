...
p <- ggplot(tab_qg, aes(x = 1, y = prop, fill = qg_label)) +
  ...
  labs(
    x = NULL, y = NULL,
    title = "Share of iNaturalist records by quality grade",
    subtitle = paste(
      "Before, During, and After BioBlitz events —",
      "each pie sums to 100% within municipality × period panel"
    ),
    caption = paste(
      "BioFuturo na Cidade. Base: all iNaturalist records",
      "(research grade, needs ID, and casual) within fixed 30-day windows",
      "before and after each BioBlitz (excluding the event day)."
    )
  ) +
...
ggsave(png_path, p, width = 11.5, height = 8.5, dpi = 300)
message(
