...
out_tiff <- file.path(out_dir, "27_iconic_records_stacked_grouped_bars.tiff")
out_eps  <- file.path(out_dir, "27_iconic_records_stacked_grouped_bars.eps")

readr::write_csv(tab_iconic_lumped, out_csv)

ggsave(out_png,  p_iconic, width = 11, height = 6.8, units = "in", dpi = 300, bg = "white")
message("PNG saved at:  ", out_png)

ggsave(out_tiff, p_iconic, device = "tiff",
       width = 11, height = 6.8, units = "in",
       dpi = 600, compression = "lzw", bg = "white")
message("TIFF saved at: ", out_tiff)

eps_device <- if (capabilities("cairo")) grDevices::cairo_ps else "eps"
ggsave(out_eps, p_iconic, device = eps_device, width = 11, height = 6.8, units = "in")
