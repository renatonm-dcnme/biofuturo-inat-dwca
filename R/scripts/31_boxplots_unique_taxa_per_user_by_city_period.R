...
p_box_taxa <- ggplot(
  ...
  scale_y_continuous(name = "Unique taxa per unique user") +
  expand_limits(y = y_max_global * 1.12) +
  labs(
    title = "Number of unique taxa per user by municipality and period"
  ) +
...
ggsave(
  filename = file.path(out_dir, "31_boxplots_unique_taxa_per_user_by_city_period.png"),
  plot     = p_box_taxa,
  width    = 8,
  height   = 6,
  dpi      = 300
...
cat("\nFigura salva em: outputs/31_boxplots_unique_taxa_per_user_by_city_period.png\n")
