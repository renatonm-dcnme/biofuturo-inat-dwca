...
p_violin <- ggplot(lag_df, aes(x = period_en, y = lag_plot, fill = period_en)) +
  ...
  labs(
    x = NULL,
    y = "Upload lag (days)",
    title = "Discrepancy between photo date and record upload",
    subtitle = paste(
      "Violin plots of upload lag before, during, and after a Bioblitz conducted in one urban green area in each of four municipalities in southeastern Brazil.",
      "Negative lags are clipped at zero for plotting but preserved in the exported data."
    )
  ) +
...
ggsave(
  filename = "outputs/33_violin_upload_lag_by_city_period.png",
  plot     = p_violin,
  width    = 10,
  height   = 7,
  dpi      = 300
...
readr::write_csv(
  ...
  "outputs/33_upload_lag_by_observation.csv"
)

message("Done! Violin plot and lag table saved in 'outputs/'.")
